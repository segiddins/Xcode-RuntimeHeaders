//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <DVTUserInterfaceKit/DVTOutlineView.h>

@interface _TtC16DVTExplorableKit22DVTExplorerOutlineView : DVTOutlineView
{
    // Error parsing type: , name: highlightFragments
    // Error parsing type: , name: oneLineCellID
    // Error parsing type: , name: multiLineCellID
    // Error parsing type: , name: processingMouseEvent
    // Error parsing type: , name: processingKeyEvent
}

- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;
- (BOOL)dvt_interceptDisclosureButtonAction:(id)arg1;
- (id)makeViewWithIdentifier:(id)arg1 owner:(id)arg2;
- (struct CGRect)frameOfOutlineCellAtRow:(long long)arg1;
- (void)mouseDown:(id)arg1;
- (void)keyDown:(id)arg1;

@end

