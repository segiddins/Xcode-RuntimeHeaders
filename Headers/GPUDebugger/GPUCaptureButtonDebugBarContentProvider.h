//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <GPUDebugger/GPUDebugBarContentProvider.h>

#import <GPUDebugger/GPUCaptureLongClickPopUpButtonDelegate-Protocol.h>
#import <GPUDebugger/NSMenuDelegate-Protocol.h>

@class DVTObservingToken, GPUCaptureLongClickPopUpButton, NSMenuItem, NSString;

__attribute__((visibility("hidden")))
@interface GPUCaptureButtonDebugBarContentProvider : GPUDebugBarContentProvider <NSMenuDelegate, GPUCaptureLongClickPopUpButtonDelegate>
{
    GPUCaptureLongClickPopUpButton *_captureFrameButton;
    NSMenuItem *_captureFrameMenuItem;
    NSMenuItem *_captureScopeMenuItem;
    DVTObservingToken *_originalAppDiedObservation;
    DVTObservingToken *_debuggingAdditionObservation;
    DVTObservingToken *_debuggingControllerStateObserver;
    DVTObservingToken *_gpuCaptureUnavailReasonObservation;
    id _windowDidBecomeMainObserver;
    id _activeTabControllerChangedObserver;
    id _debugMenuChangedObserver;
    BOOL _stopCaptureButtonClicked;
}

+ (id)assetBundle;
- (void).cxx_destruct;
- (void)menuWillOpen:(id)arg1;
- (void)_populateCaptureStartMenu:(id)arg1 forDebuggerController:(id)arg2;
- (void)_disableCaptureMenu;
- (void)_enableCaptureMenuToStop;
- (void)_enableCaptureMenuToStart;
- (void)_initCaptureMenu;
- (id)_capturableObjectsInfoForDebuggerController:(id)arg1;
- (void)stopDebugSession:(id)arg1;
- (void)stopCaptureGPUFrame:(id)arg1;
- (void)captureGPUFrame:(id)arg1;
- (void)resumeGPUFrame:(id)arg1;
- (void)_debugMenuChanged:(id)arg1;
- (void)_activeTabControllerChanged:(id)arg1;
- (void)_rebindMenuItems;
- (void)_windowDidBecomeMain:(id)arg1;
- (void)willBeDetachedFromDebugBar:(id)arg1;
- (id)_findMenuItemWithRepresentedObject:(id)arg1 inMenu:(id)arg2;
- (void)_setUpCaptureGPUFrameMenuItem:(id)arg1;
- (void)wasAttachedToDebugBar:(id)arg1;
- (BOOL)validateMenuItem:(id)arg1;
- (void)updateMenus;
- (void)updateCaptureStates;
- (void)updateStateRESUME;
- (void)updateStateCaptureSTOP;
- (void)updateStateSTOP;
- (void)updateStateSTART;
- (void)updateStateERROR;
- (void)_disableCaptureButton;
- (void)_enableCaptureButton;
- (id)updatedMenuForButton:(id)arg1;
- (void)captureFrameFromMenu:(id)arg1;
- (void)setCaptureButtonToCaptureStop;
- (void)setCaptureButtonToCapture;
- (void)setCaptureButtonToStop;
- (void)setCaptureButtonToResume;
- (void)currentDebugSessionStateDidChange;
- (void)currentDebugSessionDidChange;
- (void)workspaceDidFinishLoading;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

