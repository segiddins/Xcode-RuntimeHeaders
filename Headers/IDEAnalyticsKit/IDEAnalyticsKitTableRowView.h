//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <AppKit/NSTableRowView.h>

@class NSColor, NSTrackingArea;
@protocol IDEAnalyticsKitTableRowViewMouseInsideDelegate;

@interface IDEAnalyticsKitTableRowView : NSTableRowView
{
    NSTrackingArea *_trackingArea;
    BOOL _alwaysDrawSeparatorLine;
    id <IDEAnalyticsKitTableRowViewMouseInsideDelegate> _mouseInsideDelegate;
    NSColor *_rowGridLineColor;
    CDStruct_2b6e6e30 _rowGridLineInset;
}

- (void).cxx_destruct;
@property(nonatomic) BOOL alwaysDrawSeparatorLine; // @synthesize alwaysDrawSeparatorLine=_alwaysDrawSeparatorLine;
@property(retain) NSColor *rowGridLineColor; // @synthesize rowGridLineColor=_rowGridLineColor;
@property(nonatomic) CDStruct_2b6e6e30 rowGridLineInset; // @synthesize rowGridLineInset=_rowGridLineInset;
@property(nonatomic) __weak id <IDEAnalyticsKitTableRowViewMouseInsideDelegate> mouseInsideDelegate; // @synthesize mouseInsideDelegate=_mouseInsideDelegate;
- (void)mouseExited:(id)arg1;
- (void)mouseEntered:(id)arg1;
- (void)updateTrackingAreas;
- (struct CGRect)_separatorRect;
- (id)separatorColor;
- (BOOL)drawsSeparator;
- (void)prepareForReuse;
- (BOOL)isOpaque;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;
- (void)dvt_commonInit;

@end

