//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <DVTFoundation/DVTInvalidation_NSObject.h>

@class MISSING_TYPE;

__attribute__((visibility("hidden")))
@interface _TtCC13IDEFoundation22IDEBuildNoticeProvider17SchemeLogObserver : DVTInvalidation_NSObject
{
    MISSING_TYPE *identifiedRoot;
    MISSING_TYPE *sectionAddedObservation;
    MISSING_TYPE *sectionSealedObservation;
    MISSING_TYPE *blueprintSessions;
    MISSING_TYPE *completedChildSections;
    MISSING_TYPE *observer;
}

- (void).cxx_destruct;
- (id)init;
- (void)primitiveInvalidate;

@end

